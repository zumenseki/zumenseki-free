...
import { Upload, FileText, Crown, AlertTriangle, Ruler } from "lucide-react"
...
export default function PDFAreaCalculator() {
  ...
  const [upgradeReason, setUpgradeReason] = useState<'length' | 'storage' | 'filesize'>('length')

  // 現在のページの測定結果を取得 (常に1ページ目)
  const currentPageMeasurements = allMeasurements.filter((m) => m.pageNumber === 1)

  // フリーミアム制限チェック関数
  const showUpgradeModal = (reason: 'length' | 'storage' | 'filesize') => {
