...
import { Upload, FolderOpen, Trash2, FileText, Calendar, HardDrive, Crown, AlertTriangle, Ruler, Save, Zap } from "lucide-react"
...
export default function PDFAreaCalculator() {
  ...
  // 残り測定回数を取得
  const remainingMeasurements = getRemainingMeasurements()
  ...
  const drawOverlay = useCallback(() => {
    ...
    currentPageMeasurements.forEach((measurement) => {
      if (measurement.points.length > 2) {
        ...
        ctx.strokeText(measurement.name, centerX, centerY)
        ctx.fillText(measurement.name, centerX, centerY)
        ctx.font = "bold 14px Arial"
        const areaText = `${measurement.area} ${measurement.unit}²`
        ctx.strokeText(areaText, centerX, centerY + 25)
        ctx.fillText(areaText, centerX, centerY + 25)
      }
    })
  ...
  }, [points, scalePoints, scaleMode, currentPageMeasurements])
  ...
  return (
    <div className="container mx-auto p-4 max-w-6xl">
      ...
      <div className="mb-6">
        ...
        <UpgradePrompt 
          variant="header" 
          onUpgradeClick={() => showUpgradeModal('measurements')} 
        />
        ...
        {/* 制限警告 */}
        {remainingMeasurements <= 1 && remainingMeasurements > 0 && (
          <div className="mb-4">
            <UpgradePrompt 
              variant="limit-warning" 
              remainingCount={remainingMeasurements}
              onUpgradeClick={() => showUpgradeModal('measurements')} 
            />
          </div>
        )}
            ...
            <>
              <div className="mb-4">
                ...
                <div className="text-sm text-gray-600">
                  ファイルサイズ: {(pdfFile.size / (1024 * 1024)).toFixed(2)}MB | 1ページ目を表示中
                </div>
              </div>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      <div className="grid grid-cols-1 gap-2">
                        ...
                        {/* 長さ測定ボタン（プレミアム機能として強調） */}
                        <div className="relative">
                          <Button
                            variant="outline"
                            onClick={toggleLengthMode}
                            className="w-full text-sm border-amber-300 hover:bg-amber-50 relative"
                            disabled
                          >
                            <Ruler className="w-4 h-4 mr-2" />
                            長さ測定
                      ...
                      <div className="mt-3">
                        <UpgradePrompt 
                          ...
                          onUpgradeClick={() => showUpgradeModal('length')} 
                        />
                      </div>
